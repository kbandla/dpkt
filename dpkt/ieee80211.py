...
def test_802211_ack():
    ...
    assert ieee.wep == 0
    assert ieee.order == 0
    assert ieee.ack.dst == '\x00\x12\xf0\xb6\x1c\xa4'
...
def test_80211_beacon():
    ...
    assert ieee.wep == 0
    assert ieee.order == 0
    assert ieee.mgmt.dst == '\xff\xff\xff\xff\xff\xff'
    assert ieee.mgmt.src == '\x00\x26\xcb\x18\x6a\x30'
    assert ieee.beacon.capability == 0x3104
    assert ieee.capability.privacy == 1
    assert ieee.ssid.data == 'CAEN'
    assert ieee.rate.data == '\x82\x84\x8b\x0c\x12\x96\x18\x24'
    assert ieee.ds.data == '\x01'
    assert ieee.tim.data == '\x00\x01\x00\x00'
    fcs = struct.unpack('I', s[-4:])[0]
    assert ieee.fcs == fcs
...
def test_80211_data_qos():
    ...
    assert ieee.data_frame.dst == '\x00\x26\xcb\x17\x44\xf0'
    assert ieee.data_frame.src == '\x00\x23\xdf\xc9\xc0\x93'
    assert ieee.data_frame.frag_seq == 0x207b
    assert ieee.data == ''
    assert ieee.qos_data.control == 0x0
    assert ieee.fcs == struct.unpack('I', '\xff\xff\xff\xff')[0]
...
def test_data_ds():
    ...
    assert ieee.data_frame.sa == '\x00\x26\xcb\x18\x73\x50'
    assert ieee.data_frame.src == '\x00\x26\xcb\x18\x73\x50'
    assert ieee.data_frame.dst == '\x01\x0b\x85\x00\x00\x00'
    assert ieee.data_frame.da == '\x01\x0b\x85\x00\x00\x00'
    ...
    assert ieee.to_ds == 1
    assert ieee.from_ds == 0
    assert ieee.data_frame.bssid == '\x00\x26\xcb\x17\x48\xc1'
    assert ieee.data_frame.src == '\x00\x24\x2c\xe7\xfe\x8a'
    assert ieee.data_frame.dst == '\xff\xff\xff\xff\xff\xff'
    ...
    assert ieee.to_ds == 0
    assert ieee.from_ds == 1
    assert ieee.data_frame.bssid == '\x00\x1f\x33\x39\x75\x44'
    assert ieee.data_frame.src == '\x00\x1f\x33\x39\x75\x44'
    assert ieee.data_frame.dst == '\x00\x02\x44\xac\x27\x70'
...
def test_compressed_block_ack():
    ...
    assert ieee.back.dst == '\x34\xc0\x59\xd6\x3f\x62'
    assert ieee.back.src == '\xb4\x75\x0e\x46\x83\xc1'
